...
class Peer:
    ...
    def processar_mensagens(self):
        while self.receptor_ativo: # Aguarda o receptor ser desativado
            ...
            if mensagens_para_processar:
                self.log(f"Verificando suas mensagens: {[str(mensagem[:5]) + '...' for mensagem in mensagens_para_processar]}")
                    ...
                    try:
                        ...
                        match mensagem.get("tipo"):
                            ...
                            case "REGISTRO":
                                ...
                                self.conectar_servidor(peer_porta)
                            ...
                            case "ESTOQUE":
                                ...
                                if not ((peer_id, peer_estoque)) in self.estoques:
                                    with self.lock:
                                        self.estoques.append((peer_id, peer_estoque))
                                else:
                                    for estoque in self.estoques:
                                        if estoque[0] == peer_id:
                                            with self.lock:
                                                estoque[1] = peer_estoque
                                                break
                                        
                                print(f"Estoque do peer {peer_id} recebido") 
                            ...
                            case "PEDIDO":
                                ...
                                if bloco_solicitado in self.indices:
                                    self.enviar_bloco(peer_id, bloco_solicitado)
    ...
    def solicitar_bloco(self, peer_alvo, bloco):
        ...
        conexao = self.peers[peer_alvo].get("conexao")
        conexao.send(json.dumps(mensagem)).encode()
        
    def enviar_bloco(self, peer_alvo, bloco):
        mensagem = {
            "tipo": "BLOCO",
            "id": self.id,
            "bloco":bloco
        ...
        conexao = self.peers[peer_alvo].get("conexao")
        conexao.send(json.dumps(mensagem)).encode()
        print(f"Bloco Enviado para o peer {peer_alvo}")
